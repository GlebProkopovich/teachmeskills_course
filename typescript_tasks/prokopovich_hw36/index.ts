...
const getGenres = (arr: IFilm[]): string[] => {
  return arr
  .map(el => el.genre)
  .join(',')
  .split(',')
...
const getActors = (arr: IFilm[]): string[] => {
  return arr
  .map(el => el.actors)
  .join(',')
  .split(',')
...
const getSortedByRatingFilms = (arr: IFilm[]): IFilm[] => {
...
const getNewFilms = (arr: IFilm[]): INewFilm[] => {
...
const getFilmsByReleasedYear = (arr: IFilm[], year: number): IFilm[] => {
  return arr.filter(el => el.year === year);
}

// 6. Создать функцию, которая бы принимала массив фильмов и строку. А результатом этой функции должен быть
// новый отфильтрованный массив, с фильмами, где строка входит в название фильма.

const getFilmsByTitle = (arr: IFilm[], title: string): IFilm[] => {
  return arr.filter(el => el.title.toLocaleLowerCase().includes(title.toLowerCase()));
}
...
const getFilmsByTitleOrPlot = (arr: IFilm[], str: string): IFilm[] => {
  return arr
  .filter(el => el.title
    .toLocaleLowerCase()
    .includes(str) 
...
// 1) массив фильмов
// 2) строка (название поля, например 'title') и строку/число (значение поля 'Black Widow').
...
const getFilmsByEntries = (arr: IFilm[], key: string, value: string | number): IFilm[] => {
  return arr
  .filter(el => 
